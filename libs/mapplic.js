	...
	var Mapplic = function() {
		...
		function Deeplinking() {
            ...
            $('body').on('click','.mapplic-pin', function(e) {
                var actualPin = $(e.currentTarget);
                var id = actualPin.data('pin');
					...
					$.each(value.locations, function(index, value) {
						...
						if (value.pin != 'hidden') {
							if (self.o.locations) {
								...
								var pin = $('<a>'+img+'</a>').attr('data-pin', target).addClass('mapplic-pin').css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
		...
		var addControls = function() {
			...
			// Mousewheel   - momentaneamente eliminato
		...
		var moveTo = function(x, y, scale, d, easing) {
			if (scale !== undefined) {
                ...
                //Manage icons size
                /*var pinWidth = 100*scale;
                $('.mapplic-pin').find('img').css({width:pinWidth+'%'});*/
			}
			else {
				self.map.css({
					'left': x,
